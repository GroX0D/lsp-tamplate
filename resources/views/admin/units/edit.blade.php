...
@section('title')
    <div>
        ...
        <h1 class="text-2xl font-semibold text-slate-900">Edit Unit Kompetensi</h1>
    </div>
@endsection
    ...
    <div class="rounded-3xl bg-white border border-slate-200 p-6 shadow-sm">
        <form action="{{ route('admin.units.update', $unit) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="kode_unit" value="{{ old('kode_unit', $unit->kode_unit) }}" class="mt-2 w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900" required>
            ...
            <div>
                ...
                <input type="text" name="nama_unit" value="{{ old('nama_unit', $unit->nama_unit) }}" class="mt-2 w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900" required>
            ...
            <div>
                ...
                <select name="skema_id" class="mt-2 w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900" required>
                    ...
                    @foreach($skemas as $skema)
                        <option value="{{ $skema->id }}" {{ old('skema_id', $unit->skema_id) == $skema->id ? 'selected' : '' }}>{{ $skema->nama_skema }}</option>
                    @endforeach
                </select>
